...
if(isset($_POST['update'])){
    ...
    $q = "UPDATE patients SET name='$name', email='$email', mobile='$mobile', address='$address', blood_group='$bg' WHERE id='$id'";
    if(mysqli_query($connection, $q)){
        echo "<script>alert('✅ Patient Updated'); window.location.href='manage_users.php?tab=patients';</script>";
    } else {
...
$id = $_GET['id'];
$row = mysqli_fetch_assoc(mysqli_query($connection, "SELECT * FROM patients WHERE id='$id'"));
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        .btn-custom { background: #1d3557; color: white; height: 50px; border-radius: 8px; font-weight: 600; width: 100%; border: none; transition: 0.3s; }
        .btn-custom:hover { background: #457b9d; transform: translateY(-2px); }
    </style>
</head>
<body>
    <div class="form-card">
        <h3 class="font-weight-bold mb-4 text-center" style="color: #1d3557;">Edit Patient</h3>
        <form method="post">
            ...
            <button type="submit" name="update" class="btn btn-custom mt-3">Update Patient</button>
