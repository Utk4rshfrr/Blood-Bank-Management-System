...
<head>
    ...
    <style>
        ...
        .nav-tabs .nav-link:hover { color: #e63946; }
    ...
    <div class="container py-5">
        <div class="main-card">
            ...
            <ul class="nav nav-tabs">
                <li class="nav-item">
                    <a class="nav-link <?php echo ($active_tab == 'donors') ? 'active-tab' : ''; ?>" href="manage_donations.php?tab=donors">🩸 By Donors</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link <?php echo ($active_tab == 'patients') ? 'active-tab' : ''; ?>" href="manage_donations.php?tab=patients">🏥 By Patients</a>
                </li>
            </ul>
            ...
            <div class="table-responsive">
                <table class="table custom-table">
                    ...
                    <tbody>
                        <?php
                        // LOGIC FOR DONORS OR PATIENTS
                        if($active_tab == 'donors'){
                            // FIX: Uses 'id' instead of 'donation_id' for legacy table
                            $query = "SELECT * FROM donation ORDER BY id DESC";
                            $id_col = 'donor_id';
                            $pk = 'id'; // Primary Key is 'id'
                            $type = 'donor_donation';
                        } else {
                            // New table uses 'donation_id'
                            $query = "SELECT * FROM patient_donation_offers ORDER BY donation_id DESC";
                            $id_col = 'patient_id';
                            $pk = 'donation_id'; // Primary Key is 'donation_id'
                        ...
                        if(mysqli_num_rows($result) > 0){
                            while($row = mysqli_fetch_assoc($result)){
                                ...
                                if($st == 1 || $st == 'Approved') { $st = 'Approved'; $badge_class = 'badge-approved'; }
                                elseif($st == 2 || $st == 'Rejected') { $st = 'Rejected'; $badge_class = 'badge-rejected'; }
                                elseif($st == 0) { $st = 'Pending'; }

                                // Handle Units display (some tables might differ)
                                $units = isset($row['no_units']) ? $row['no_units'] : (isset($row['available_units']) ? $row['available_units'] : '-');
                                
                                // Handle Blood Group display
                                $bg = isset($row['blood_group']) ? $row['blood_group'] : '-';

                                echo "<tr>
                                    <td>#{$row[$pk]}</td>
                                    <td>User #{$row[$id_col]}</td>
                                    <td class='font-weight-bold text-danger'>{$bg}</td>
                                    <td>{$units}</td>
                                    <td><span class='badge-custom {$badge_class}'>{$st}</span></td>
                                    <td class='text-center'>";
                                    
                                if($st == 'Pending'){
                                    echo "<a href='action_request.php?id={$row[$pk]}&type={$type}&action=approve' class='btn btn-success btn-sm' style='border-radius:50px; padding:5px 15px;'>Approve</a> ";
                                    echo "<a href='action_request.php?id={$row[$pk]}&type={$type}&action=reject' class='btn btn-light text-danger btn-sm' style='border-radius:50px; padding:5px 15px;'>Reject</a>";
                                } else {
                            ...
                            echo "<tr><td colspan='6' class='text-center py-4 text-muted'>No records found.</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
